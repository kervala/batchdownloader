...
public:
	...
	void addEntry(EntryType type, const QString &fileName, const QByteArray &contents, const QDateTime &lastModified);
};
...
void QZipWriterPrivate::addEntry(EntryType type, const QString &fileName, const QByteArray &contents, const QDateTime &lastModified)
{
#ifndef NDEBUG
    ...
    writeMSDosDate(header.h.last_mod_file, lastModified);
...
bool QZipReader::extractAll(const QString &destinationDir) const
{
    ...
    for (const FileInfo &fi : allFiles) {
        ...
        if (fi.isFile) {
			...
			setFileModificationDate(absPath, fi.lastModified);
		}
    }
...
/*!
Extracts a file into \a destinationDir on the local filesystem.
In case writing or linking a file fails, the extraction will be aborted.
*/
bool QZipReader::extractFile(int index, const QString &destinationDir) const
{
	// check if destinationDir is a directory or a file
	bool isFile = destinationDir.indexOf('.') > -1;

	FileInfo fi = entryInfoAt(index);

	// don't extract directories or symbolic links
	if (fi.isSymLink || fi.isDir) return false;

	QString absPath = destinationDir;
	
	if (!isFile) absPath += QDir::separator() + fi.filePath;
	...
	if (fi.isFile)
	{
		...
		QByteArray data = fileData(fi.filePath);
		qint64 written = f.write(data);

		// check if the whole file is written
...
/*!
    ...
    \sa setCompressionPolicy()
*/
void QZipWriter::addFile(const QString &fileName, const QByteArray &data, const QDateTime &lastModified)
{
    d->addEntry(QZipWriterPrivate::File, QDir::fromNativeSeparators(fileName), data, lastModified);
}
...
/*!
    ...
    includes the full path in the archive.
*/
void QZipWriter::addFile(const QString &fileName, QIODevice *device, const QDateTime &lastModified)
{
    Q_ASSERT(device);
    ...
    d->addEntry(QZipWriterPrivate::File, QDir::fromNativeSeparators(fileName), device->readAll(), lastModified);
    if (opened)
        device->close();
}
...
/*!
    ...
    the \a permissions;
*/
void QZipWriter::addDirectory(const QString &dirName, const QDateTime &lastModified)
{
    QString name(QDir::fromNativeSeparators(dirName));
    // separator is mandatory
    if (!name.endsWith(QLatin1Char('/')))
        name.append(QLatin1Char('/'));
    d->addEntry(QZipWriterPrivate::Directory, name, QByteArray(), lastModified);
}
...
*/
void QZipWriter::addSymLink(const QString &fileName, const QString &destination, const QDateTime &lastModified)
{
    d->addEntry(QZipWriterPrivate::Symlink, QDir::fromNativeSeparators(fileName), QFile::encodeName(destination), lastModified);
}
